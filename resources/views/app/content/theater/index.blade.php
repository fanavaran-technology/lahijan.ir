...
@section('content')
    <section class="container mt-5 min-h-screen">
        ...
        <section class="grid grid-cols-12 gap-6">
            @foreach ($theaters as $theater)
                <section
                    ...
                    <a href="{{ $theater->publicPath() }}">
                        <img class="rounded-lg w-full h-44 object-cover" src="{{ asset($theater->image) }}"
                             alt="{{ $theater->title }}" />
                        <h5 class="text-gray-900 text-base md:text-base lg:text-lg font-medium my-4">
                            {{ Str::limit($theater->title, 100, '...') }}
                        </h5>
                    </a>
                    <section class="flex justify-between items-center">
                        <div class="flex flex-col sm:flex-row justify-center items-center sm:space-x-4">
                            <div class="font-medium">
                                <div class="text-gray-500 text-xs mx-2">{{ jalaliDate($theater->created_at, '%d %B، %Y') }}
                                </div>
                            </div>
                        </div>
                        <a href="{{ $theater->publicPath() }}" class="flex items-center">
                            <span
        ...
        <section class="overflow-x-hidden mt-10 flex justify-center">
            {{ $theaters->appends($_GET)->render('vendor.pagination.tailwind') }}
        </section>
    </section>
@endsection
