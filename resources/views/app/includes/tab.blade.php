        ...
        <div class="swiper mySwiper">
            <div class="swiper-wrapper h-60">
                @forelse ($allNews as $news )
                    @if($news->video)
                        <a href="{{ $news->publicPath() }}" class="swiper-slide hover:cursor-pointer">

                            <img class="rounded-lg object-cover"
                                 src="{{ $news->image }}">
                            <div
                                class="text-gray-50 z-50  bg-gray-900 p-1 rounded-lg text-sm absolute bottom-2 right-2">{{ Str::limit($news->title, 60, '...') }}</div>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                ...
                <div class="swiper-wrapper h-60">
                    @forelse ($reportImages as $reportImage )
                        <a href="{{ $reportImage->publicPath() }}" class="swiper-slide hover:cursor-pointer">
                            ...
                            <span style="font-family: iransans" class="z-50 flex bg-gray-800 absolute top-2 left-2 text-gray-100 rounded px-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mx-2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                          d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"/>
                                </svg>
                                {{$reportImage->gallerizable()->count()}} تصویر
                            </span>
                        </a>
                    @empty
