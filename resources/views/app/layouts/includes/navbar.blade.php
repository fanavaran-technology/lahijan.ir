<nav
    class="{{ Route::currentRouteName() == 'home' ? 'absolute' : 'border-b' }} font-vazir top-nav  z-40 w-full lg:bg-black lg:bg-opacity-70 md:bg-black bg-[#000601] bg-opacity-[83%]  md:bg-opacity-70 border-gray-200 px-2 md:px-4 py-2.5">
    <div class="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div class="flex md:order-2">
            ...
            <div class="flex items-center md:order-1">
                <button data-collapse-toggle="mega-menu" type="button"
                    class="inline-flex items-center p-2 ml-1 text-sm text-gray-500 rounded-lg md:hidden  focus:outline-none"
        ...
        <div id="mega-menu"
            ...
            <ul class="flex flex-col z-50 mt-4 font-medium md:flex-row md:space-x-4 lg:space-x-6 md:mt-0">
                @foreach ($menus->whereNull('parent_id') as $menu)
                    @if ($menu->childrens->count())
                        <li class="md:ml-4 ml-6">
                            <button id="dropdownNavbarLink" data-dropdown-toggle="{{ $loop->iteration }}-parentMenu"
                                ...
                                {{ $menu->title }}<svg class="w-4 h-4 ml-1" aria-hidden="true" fill="currentColor"
                            ...
                            <div id="{{ $loop->iteration }}-parentMenu"
                                class="z-10 hidden font-normal bg-white w-40 divide-y divide-gray-500 rounded shadow "
                                ...
                                @include('app.layouts.includes.components.head-menu-items', [
                                    'menus' => $allSubmenus,
                                ])
                            </div>
                        </li>
                    @else
                        <li class="md:ml-4 ml-6">
                            <a href="{{ empty($menu->url) ? URL::to('/') : $menu->url }}"
                                class="block font-bold whitespace-nowrap text-gray-300 border-b border-gray-500 md:border-0 hover:text-green-400 p-1 md:p-0">{{ $menu->title }}</a>
                        </li>
                    @endif
                @endforeach
            </ul>
