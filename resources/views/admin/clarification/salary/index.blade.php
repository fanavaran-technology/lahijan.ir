@extends('admin.layouts.app', ['title' => 'لیست حقوق و دستمزد'])

@section('content')
    <div class="row justify-content-center">
        <div class="col">
            <h2 class="h3 mb-0 page-title">لیست حقوق و دستمزد
                <span class="text-sm text-muted">({{ $salaries->total() }})</span>
            </h2>
        </div>
        {{-- @can('create_perssonels') --}}
            <div class="col-auto">
                <a href="{{ route('admin.clarification.salaries.create') }}" type="button" class="btn btn-primary px-4">ایجاد</a>
            </div>
        {{-- @endcan --}}
        <div class="col-12">
                            ...
                            <table class="table table-striped" id="table-id">
                                ...
                                @forelse($salaries as $salary)
                                    <tr>
                                        ...
                                        <td>
                                            <small>{{ $salary->title }}</small>
                                        </td>
                                        <td>{{ jalaliDate($salary->created_at) }}</td>
                                        <td>
                                            {{-- @can('edit_perssonels') --}}
                                                <a href="{{ route('admin.clarification.salaries.edit', $salary->id) }}"
                                            ...
                                            {{-- @can('delete_perssonels') --}}
                                                <form action="{{ route('admin.clarification.salaries.destroy', $salary->id) }}"
                                                    class="d-inline" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" x-data="{{ $salary->id }}"
