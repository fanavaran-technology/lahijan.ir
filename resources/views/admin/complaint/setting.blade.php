...
@section('content')
    ...
    <form action="{{ route('admin.complaints.settings.store') }}" method="post" enctype="multipart/form-data"
        id="form">
        @csrf
                        ...
                        <div class="row">
                            <div class="form-group col-lg-4">
                                ...
                                <input type="number" name="max-file-size" value="{{ $config['max-file-size'] }}"
                                    placeholder="حداکثر حجم فایل" dir="ltr" class="form-control custom-focus"
                                    id="full_name">
                            </div>
                            <div class="form-group col-lg-4">
                                <label for="full_name" class="input-title">حداکثر تعداد فایل</label>
                                <input type="number" name="max-files" value="{{ $config['max-files'] }}"
                                    placeholder="حداکثر تعداد فایل" dir="ltr" class="form-control custom-focus"
                                    id="full_name">
                            </div>
                            <div class="form-group col-lg-4">
                                <label for="full_name" class="input-title">مهلت پاسخ به شکایات (بر اساس روز)</label>
                                <input type="number" name="deadline-responding"
                                    value="{{ $config['deadline-responding'] }}" placeholder="حداکثر تعداد فایل"
                                    dir="ltr" class="form-control custom-focus" id="full_name">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-check form-check-inline col-md-8 mx-0 my-3">
                                <input id="confirm_referrer" name="confirm_referrer" value="1"
                                    data-width="100" data-off="غیرفعال" data-on="فعال" data-onstyle="success"
                                    data-offstyle="danger" class="form-check-input" type="checkbox" data-toggle="toggle"
                                    data-style="mr-1" @checked($config['confirm_referrer'])>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="form-check form-check-inline mx-0 my-3">
                                    <input id="send_sms_plaintiff" name="notifications[send_sms_plaintiff]"
                                        ...
                                        @checked($config['notifications']['send_sms_plaintiff'])>
                                    <label for="send_sms_plaintiff" class="form-check-label pr-2 pl-0">ارسال sms برای
                                        اطلاع
                                        رسانی شاکی </label>
                                </div>
