@extends('admin.layouts.app', ['title' => 'ویرایش فراخوان '])
...
@section('content')
    <div class="row d-flex justify-content-between">
      <div class="d-flex justify-content-between align-items-center">
          <h2 class="h3 mb-0 section-heading">ویرایش فراخوان</h2>
      </div>
      <div class="col-auto mb-3">
        <a href="{{ route('admin.content.public-calls.index') }}" type="button" class="btn btn-success px-4">بازگشت</a>
      </div>
    </div>
    @if ($errors->any())
    ...
    @endif
    <form action="{{ route('admin.content.public-calls.update' , $publicCall->id) }}" method="post" enctype="multipart/form-data" id="form">
        @csrf
        @method('PUT')
        <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-row">
                            <div class="form-group col-md-12 my-2">
                                <input type="text" name="title" value="{{ old('title' , $publicCall->title) }}" onkeyup="copyToSlug(this)"
                                    placeholder="عنوان را اینجا وارد کنید"
                                    class="form-control custom-input-size custom-focus" id="title">
                            </div>
                            <div class="col-12 slug d-flex">
                                <span>https://lahijan.ir/public-calls/</span>
                                <span class="slug-box"></span>
                            </div>
                            <div class="form-group col-md-12 my-2">
                                <textarea name="description" id="editor">{{ old('description' , $publicCall->description) }}</textarea>
                            </div>
                        </div>
                    </div> <!-- /. col -->
                    <!-- end places content -->
            ...
            <div class="col-12 col-md-3 my-2 px-0">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <img src="{{ asset($publicCall->image) }}" style="width: 6rem;" alt="">
                    </div>
                </div>
                <div class="card mt-2">
                    <div class="card-header" onclick="openCard(this)">
                    ...
                    <div class="card-body">
                        <div class="mt-2">
                            <label for="event_date_view" class="input-title">
                                تعیین تاریخ رویداد
                            </label>
                            <input type="hidden" name="event_date" id="event_date" value="{{ old('event_date' , $publicCall->event_date) }}">
                            <input id="event_date_view" class="form-control custom-focus" value="{{ old('event_date' , $publicCall->event_date) }}">
                        </div>
                        ...
                        <div class="mt-2">
                            ...
                            <input type="hidden" name="published_at" id="published_at" value="{{ old('published_at' , $publicCall->published_at) }}">
                            <input id="published_at_view" class="form-control custom-focus" value="{{ old('published_at' , $publicCall->published_at) }}">
                        </div>
                        <div class="form-group mt-4 custom-control custom-checkbox ">
                            <input type="checkbox" name="status" value="1" @checked(old('status' , $publicCall->status))
                                class="custom-control-input" id="status">
                            <label class="custom-control-label input-title" for="status">این فراخوان فعال باشد</label>
                        </div>
                    </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#event_date_view').persianDatepicker({
                altField: '#event_date',
                format: 'YYYY/MM/DD',
                minDate: "today",
                timePicker: {
