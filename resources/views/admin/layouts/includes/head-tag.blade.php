<meta charset="utf-8">


<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
<meta name="description" content="">
<meta name="author" content="">
<link rel="icon" href="{{ asset(Setting::getValue('logo')) }}">


<link rel="stylesheet" href="{{ asset('assets/admin/notification/fonts/icomoon/style.css') }}">
<link rel="stylesheet" href="{{ asset('assets/admin/notification/css/style.css') }}">
<link rel="stylesheet" href="{{ asset('assets/admin/notification/css/owl.carousel.min.css') }}">

<link rel="stylesheet" href="{{ asset('assets/admin/plugins/simple-notify/simple-notify.min.css') }}">
<link rel="stylesheet" href="{{ asset('assets/admin/plugins/jquery-confirm/jquery-confirm.min.css') }}">
<!-- Simple bar CSS -->
<link rel="stylesheet" href="{{ asset('assets/admin/css/simplebar.css') }}">
<!-- Icons CSS -->
<link rel="stylesheet" href="{{ asset('assets/admin/css/fonts.css') }}">
<link rel="stylesheet" href="{{ asset('assets/admin/css/style.css') }}">
<link rel="stylesheet" href="{{ asset('assets/admin/css/feather.css') }}">
<meta name="csrf-token" content="{{ csrf_token() }}">
<link rel="stylesheet" href="{{ asset('assets/admin/css/app-light.css') }}" id="lightTheme">

<link rel="stylesheet" href="{{ asset('assets/admin/css/custom.css') }}">
